...
export const createArticle = async (
  ...
  try {
    ...
    if (userDetails.status !== "ACTIVE") {
      res.status(403).json(errorResponse("Your account is not active."));
      return;
    }
    ...
    if (!userDetails.isEmailVerified) {
      res
        .status(403)
        .json(errorResponse("Verify your email before creating articles."));
      return;
    }
    ...
    const {
      ...
      tags,
      isPublic,
      groupId, // still allow optional groupId
    } = req.body;

    const articleIsPublic = isPublic === true || isPublic === "true";
    const userIsPresident = userDetails.role === RoleType.PRESIDENT;
    const userIsDivisionHead = userDetails.role !== "MEMBER";
    ...
    if (groupId) {
      ...
      if (!group) {
        res.status(400).json(errorResponse("Invalid group ID"));
        return;
      }
      ...
      if (!userIsPresident && !userIsDivisionHead && !groupMember) {
        res
          .status(403)
          .json(errorResponse("You must be a member of the group to post"));
        return;
      }
    ...
    if (req.file) {
      try {
        ...
        res.status(500).json(errorResponse("Failed to upload cover image"));
        return;
      }
    }
    ...
    const newArticle = {
      ...
      writerName: userDetails.freeName || "",
      writerEmail: userDetails.email || "",
      writerDivision: userDetails.divisionId || "",
      writerRole: userDetails.role,
      isPublic: articleIsPublic,
      isApproved: articleIsPublic ? userIsPresident : true,
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    ...
    if (!result.insertedId) {
      res.status(500).json(errorResponse("Failed to create article"));
      return;
    }
