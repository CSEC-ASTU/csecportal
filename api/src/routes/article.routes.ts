...
articleRouter.get("/best", articleController.getBestArticles); // Get best articles

// Get articles pending approval (president only)
articleRouter.get(
  "/pending-approval",
  articleController.getArticlesForApproval
);

// Get public articles (for landing page)
articleRouter.get("/public", (req, res) => {
  req.query.public = "true";
  return articleController.getAllArticles(req, res);
});

// Get private articles (for portal)
articleRouter.get("/private", (req, res) => {
  req.query.public = "false";
  return articleController.getAllArticles(req, res);
});

// Approve or reject an article (president only)
articleRouter.patch("/:articleId/approve", articleController.approveArticle);
